        ...
        for rank_file in ranked_file:
            ...
            with open(rank_file, "r") as f:
                ...
                for line in f:
                    ...
                    choice_list = [parts[i] for i in range(len(parts))]
                    # Docking score column: 5 cols (with lig efficiency) -> -3, 4 cols -> -2, 3 cols -> -1
                    ...
                    if len(choice_list) < 3:
                        continue
                    try:
                        gen_affinity_sum = gen_affinity_sum + float(choice_list[score_idx])
                    except (ValueError, TypeError):
                        continue
                    num_lines_counter = num_lines_counter + 1.0

            gen_affinity_average = (
                gen_affinity_sum / num_lines_counter if num_lines_counter > 0 else 0.0
            )
        ...
        for rank_file in ranked_file:
            ...
            if num_lines >= top_score_per_gen:
                ...
                with open(rank_file, "r") as f:
                    ...
                    gen_affinity_average = (
                        gen_affinity_sum / lines_added if lines_added > 0 else 0.0
                    )
