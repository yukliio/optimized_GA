...
def run_rank_selector(
    ...
    # Sort by chosen idx property; if score missing, put at end (sentinel)
    _sentinel = float("-inf") if reverse_sort else float("inf")

    def _sort_key_first(x):
        if isinstance(x, CompoundInfo):
            try:
                return x.score_by_index_lookup(column_idx_to_select)
            except ValueError:
                return _sentinel
        return float(x[column_idx_to_select])
    ...
    def _smiles_of(x):
        return x.smiles if isinstance(x, CompoundInfo) else x[0]

    if len(list({_smiles_of(x) for x in new_sorted_list})) >= number_to_chose:
        sorted_list = []
        smiles_list = []
        for mol_info in new_sorted_list:
            sm = _smiles_of(mol_info)
            if sm in smiles_list:
                continue
    ...
    for i in range(number_to_chose):
        mol = sorted_list[i]
        top_choice_smile_order.append(_smiles_of(mol))
