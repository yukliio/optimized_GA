...
def get_chosen_mol_full_data_list(
    ...
    def _dock_score(x):
        if isinstance(x, CompoundInfo):
            return x.docking_score if x.docking_score is not None else 0.0
        return float(x[-2])
    sorted_list = sorted(usable_list_of_smiles, key=_dock_score)
    weighted_order_list = []
...
def get_usable_format(infile: str) -> List[CompoundInfo]:
    ...
    with open(infile) as smiles_file:
        for line in smiles_file:
            ...
            # Optional: part -2 = fitness/docking, part -1 = diversity (docstring)
            docking_score = None
            ...
            if len(parts) == 3:
                ...
                try:
                    docking_score = float(parts[-2])
                except (ValueError, TypeError):
                    pass
                try:
                    diversity_score = float(parts[-1])
                except (ValueError, TypeError):
                    pass
...
def score_and_append_diversity_scores(
    ...
    # list (support both CompoundInfo and legacy list format)
    for i in range(len(fps_list_w_div_score)):
        sm = fps_list_w_div_score[i][0]
        div_score = fps_list_w_div_score[i][-1]
        for j, mol in enumerate(molecules_list):
            if mol is None:
                continue
            mol_smiles = mol.smiles if isinstance(mol, CompoundInfo) else mol[0]
            if mol_smiles == sm:
                if isinstance(mol, CompoundInfo):
                    try:
                        mol.diversity_score = float(div_score)
                    except (TypeError, ValueError):
                        mol.diversity_score = None
                else:
                    mol.append(div_score)
                break
