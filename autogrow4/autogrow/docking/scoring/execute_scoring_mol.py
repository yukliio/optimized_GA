...
def _lig_data_to_compound_info(lig: Any) -> Optional[CompoundInfo]:
    """Convert scoring result to CompoundInfo (handles legacy list format)."""
    if lig is None:
        return None
    if isinstance(lig, CompoundInfo):
        return lig
    if isinstance(lig, list) and len(lig) >= 3:
        # Legacy: [SMILES, id, Shortid, additional_info..., fitness_score]
        try:
            score_val = float(lig[-1]) if lig[-1] not in (None, "") else None
        except (TypeError, ValueError):
            score_val = None
        return CompoundInfo(
            smiles=str(lig[0]),
            name=str(lig[1]),
            short_id=str(lig[2]) if len(lig) > 2 else str(lig[1]),
...
def make_lig_score_dictionary(
    ...
    for lig in list_of_list_of_lig_data:
        lig = _lig_data_to_compound_info(lig)
        if lig is None:
            continue
